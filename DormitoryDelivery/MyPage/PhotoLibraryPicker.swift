...
final class PhotoLibraryPickerViewController: UIViewController {
    ...
    var columns: CGFloat {
        ...
        case .vertical: return UIDevice.current.userInterfaceIdiom == .pad ? 3 : 3
...
//          Assets.requestAccess { _ in
////              checkStatus(completionHandler: completionHandler)
//            self.updatePhotos()
//          }
//
        case .authorized:
...
//      if let selectedCount = collectionView.indexPathsForSelectedItems?.count, selectedCount < 2 {
...
public struct PhotoLibraryPicker: View {
    ...
    public init(_ images: Binding<[UIImage]>) {
       self._images = images
   }
    
    var saveButton: some View {
        Button(action: {
          self.images = self.assets.map { Picture(asset: $0).toImage() }
            self.presentationMode.wrappedValue.dismiss()
        }, label: { Text("확인") }).disabled(assets.isEmpty)
       }
    ...
    public var body: some View {
        NavigationView {
            ViewControllerWrapper(assets: $assets)
               .navigationBarTitle(Text("사진"), displayMode: .inline)
...
//    }
  public func toImage(width: Int = 360, height: Int = 360, mode: PHImageContentMode = .aspectFit) -> UIImage {
      let manager = PHImageManager.default()
      let option = PHImageRequestOptions()
      var image = UIImage()
      option.isSynchronous = true
    ...
    print(asset)
//    manager.requestImage(for: asset, targetSize: CGSize(width: width, height: height), contentMode: mode, options: option, resultHandler: {(result, info)->Void in
    manager.requestImage(for: asset, targetSize: PHImageManagerMaximumSize, contentMode: PHImageContentMode.default, options: option, resultHandler: {(result, info)->Void in
        print(result, info)
          image = result!
      })
      return image
