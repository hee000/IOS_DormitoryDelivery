...
func urlimgdownloadurl(rid: String) -> URL {
  let url = URL(string: "http://192.168.35.165:3000/room/" + rid + "/purchase-screenshot-urls")!
  return url
}
...
func urluniversity() -> URL {
  let url = URL(string: "http://192.168.35.165:3000/university")!
  return url
}

func urluniversitydormitory(id: String) -> URL {
  let url = URL(string: "http://192.168.35.165:3000/university/" + id + "/dormitory")!
  return url
}


func urlemailsend() -> URL {
  let url = URL(string: "http://192.168.35.165:3000/auth/email/send")!
  return url
}

func urlemailverify() -> URL {
  let url = URL(string: "http://192.168.35.165:3000/auth/email/verify")!
  return url
}

func urlsession() -> URL {
  let url = URL(string: "http://192.168.35.165:3000/auth/token")!
  return url
}


func urlwithdrawal(uid: String) -> URL {
  let url = URL(string: "http://192.168.35.165:3000/uesr/\(uid)")!
  return url
}
